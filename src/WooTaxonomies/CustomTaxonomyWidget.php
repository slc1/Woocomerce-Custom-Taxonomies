 ...
 * @extends WC_Widget
 */
class CustomTaxonomyWidget extends \WC_Widget
{
    ...
    /**
     * CustomTaxonomyWidget constructor.
     * @param $taxonomySlug
     */
    public function __construct($taxonomySlug, $taxonomyName)
    {
        $this->taxonomySlug = $taxonomySlug;
        $this->widget_cssclass = 'woocommerce widget_product_' . $this->taxonomySlug;
        $this->widget_description = __('A list or dropdown of product categories.', 'woocommerce');
        $this->widget_id = 'woocommerce_product_' . $this->taxonomySlug;
        $this->widget_name = __('Product', 'woocommerce') . ' ' . $taxonomyName;
        $this->settings = array(
            'title' => array(
                'type' => 'text',
                'std' => __('Product', 'woocommerce') . ' ' . $taxonomyName,
    ...
    public function widget($args, $instance)
    {
        ...
        if (is_tax($this->taxonomySlug)) {
            $this->current_cat = $wp_query->queried_object;
            $this->cat_ancestors = get_ancestors($this->current_cat->term_id, $this->taxonomySlug);
            ...
            if ($terms) {
                ...
                $this->cat_ancestors = get_ancestors($main_term->term_id, $this->taxonomySlug);
            }
        }
        ...
        if ($dropdown) {
            ...
            $list_args['walker'] = new ListWalker($this->taxonomySlug);
            $list_args['title_li'] = '';
